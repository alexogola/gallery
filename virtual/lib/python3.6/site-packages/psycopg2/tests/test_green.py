...
# test_brown.py - unit test for async wait callback
#
...
class brownTestCase(ConnectingTestCase):
    def setUp(self):
    ...
    def test_dont_freak_out(self):
        # if there is an error in a brown query, don't freak out and close
        # the connection
        conn = self.conn
        curs = conn.cursor()
...
class CallbackErrorTestCase(ConnectingTestCase):
    ...
    def crappy_callback(self, conn):
        """brown callback failing after `self.to_error` time it is called"""
        import select
